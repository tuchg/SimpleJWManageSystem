...
<script>
  ...
  import { delMySelect, fetchMyClass, fetchMySelected } from '@/api/student/course'
  ...
  export default {
    ...
    methods: {
      ...
      handleSelectionChange(val) {
        ...
        this.btnStatus = (val.length > 0) ? 'warning' : ''
      },
      ...
      handleSelected() {
        if (this.multipleSelection.length > 0) {
          ...
          delMySelect(selection).then(resp => {
            if (resp.code === 20000) {
              ...
              this.showToast('error', resp.message)
            }
          })
      ...
      async getList() {
        ...
        fetchMySelected(this.testQuery).then(response => {
          if (response.code !== 20000) {
