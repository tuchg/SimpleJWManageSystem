  ...
  export default {
    ...
    data() {
      return {
        ...
        titleOptions: this.$store.getters.titles,
        //头像上传模块
        avatarUrl: '',
        // 查询串
        testQuery: {
    ...
    created() {
      ...
      this.getList()
      //获取路由传参
      this.testQuery.title = this.$route.params.title
    ...
    methods: {
       ...
       */
      exportAPI() {
        return exportTeachers()
      },
      importAPI(data) {
        return importTeachers(data)
      },
      //根据ID匹配院系,该匹配数据在页面刷新且已登录时获取
      matchFaculty(id) {
        return this.facultyOptions.find(item => item.id === id)?.name
      },
      //根据ID匹配职称,该匹配数据在页面刷新且已登录时获取
      matchTitle(id) {
        return this.titleOptions.find(item => item.id === id)?.name
      },
      /**
       * 提醒功能
      ...
      getList() {
        ...
        fetchTeachers(this.testQuery).then(response => {
          if (response.code !== 20000) {
      ...
      // 删
      deleteData(row) {
        delTeacher({ id: row.id }).then(response => {
          if (response.code === 20000) {
