...
>
    <div class="app-container">
        ...
        >
            ...
            <el-table-column align="center" label="职称" min-width="150px">
                <template slot-scope="{row}">
                    <span>{{ row.name }}</span>
                </template>
            </el-table-column>
            <el-table-column align="center" label="职称教师" min-width="110px">
                <template slot-scope="{row}">
                    <span @click="handleSelect(row)" class="link-type">{{ row.numTitle }}</span>
                </template>
            </el-table-column>
        ...
        >
            ...
            >
                ...
                <el-form-item label="职称" prop="cname">
...
<script>
  ...
  import { createTitle, delTitle, fetchTitles, updateTitle } from '@/api/admin/title'
  ...
  export default {
    ...
    methods: {
      ...
      createData() {
        this.$refs['dataForm'].validate(valid => {
          if (valid) {
            ...
            createTitle(this.temp).then(response => {
      ...
      updateData() {
        this.$refs['dataForm'].validate(valid => {
          if (valid) {
            ...
            updateTitle(tempData).then(response => {
              if (response.code === 20000) {
      ...
      async getList() {
        ...
        fetchTitles(this.testQuery).then(response => {
          if (response.code !== 20000) {
