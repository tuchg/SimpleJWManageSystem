...
>
    <div class="app-container">
        ...
        <el-dialog
                ...
                v-el-drag-dialog
        >
            <el-table :data="studentTableData" border highlight-current-row id="classInfo" stripe
...
<script>
  ...
  import { getClassRooms } from '@/api/common'
  import {
    addMyCourse,
    delCourseStudent,
    delMyCourse,
  ...
  export default {
    ...
    methods: {
      ...
      async getList() {
        ...
        fetchMyCourses(this.testQuery).then(response => {
          if (response.code !== 20000) {
      ...
      createData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            addMyCourse(this.temp).then(response => {
      ...
      updateData() {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            updateMyCourse(tempData).then(response => {
              if (response.code === 20000) {
      ...
      // 删
      deleteData(row) {
        delMyCourse({ id: row.id }).then(response => {
          if (response.code === 20000) {
